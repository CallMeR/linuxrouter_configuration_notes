...
当 Ubuntu Server 作为物理服务器运行时，可使用 `cpufrequtils` 对 CPU 调度器进行调整。  
...
本文使用 `powersave` 调度器为演示，使用 `neovim` 编辑器来编辑 `cpufrequtils` 的配置文件。  

因为该配置文件很长，完整的配置文件可查看 [fox_sys_cpufrequtils.conf](./src/system/fox_sys_cpufrequtils.conf) 以便对比。  

修改完成后，需要重启 PVE 服务器来使参数生效。  

```bash
## 编辑 cpufrequtils 配置文件
$ sudo nvim /etc/init.d/cpufrequtils
```
