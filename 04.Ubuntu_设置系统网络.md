...
|`dnsmasq.dnsserv.conf`|`/etc/dnsmasq.d`|上游 DNS 服务器配置文件|
...
Ubuntu Server 默认使用 `systemd-resolved.service` 服务，需将其关闭来避免 `53` 端口被占用。  

关闭 `systemd-resolved.service` 服务，逐行执行以下命令。  
...
- `SmartDNS` 将绑定 `8053` 端口，并仅在回环接口 `lo` 上提供服务，请根据实际情况进行调整。  
