...
修改网络配置之前，需检查 `systemd-networkd.service` 服务状态，确保该服务开机自启动。  

若服务状态为 `enabled; preset: enabled` ，表示该服务已设置为开机自启动。  
...
若服务状态为 ` disabled; preset: enabled` ，表示该服务暂未设置为开机自启动。  
...
将 `systemd-networkd.service` 服务设置为开机自启动，执行以下命令。  

```bash
## 设置 systemd-networkd.service 开机自启动
$ sudo systemctl enable systemd-networkd.service
```

Ubuntu Server 系统默认使用 `netplan` 进行网络设置，为了避免干扰，需要将其配置文件移除。  

需同时移除 `systemd-networkd` 运行时配置文件。  
...
使用 `neovim` 编辑器创建 **拨号接口** 配置文件，执行以下命令。  
...
根据之前的内容，用于拨号的物理网口为 `enp6s18` ，拨号配置文件名为 `pppoe1` 。  
...
```bash
## 创建 PPPoE服务 配置文件
$ sudo nvim /etc/systemd/system/pppd-enp6s18@pppoe1.service
```
...
修改系统定时任务配置之前，需检查 `cron.service` 服务状态，确保该服务开机自启动。  

```bash
## 检查 cron.service
$ sudo systemctl status cron.service

## 设置 cron.service 开机自启动
...
修改防火墙配置之前，需检查 `nftables.service` 和 `sshguard.service` 服务状态，确保该服务开机自启动。  
...
修改 `Dnsmasq` 配置之前，需检查 `dnsmasq.service` 服务状态，确保该服务开机自启动。  
...
```bash
## 创建 静态地址绑定 配置文件
$ sudo nvim /etc/dnsmasq.staticv4.conf
```
...
```bash
## 创建 上游DNS服务器 配置文件
$ sudo nvim /etc/dnsmasq.d/dnsmasq.dnsserv.conf
```
...
使用 `neovim` 编辑器创建 `dnsmasq` 配置文件，执行以下命令。  
...
- 当不使用 `dnsmasq.staticv4.conf` 进行静态地址绑定时，需移除主配置文件中对应配置项。  

```bash
# This configuration file is customized by fox,
# Optimize dnsmasq parameters for Linux Router.
#
# For use Adguard Home,
...
dhcp-range=set:dnsv4,172.16.1.101,172.16.1.200,1d
...
Ubuntu 默认使用 `systemd-resolved.service` 服务，需将其关闭来避免 `53` 端口被占用。  

关闭 `systemd-resolved.service` 服务，逐条执行以下命令。  
